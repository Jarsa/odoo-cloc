[![Build Status](https://git.vauxoo.com/jarsa/odoo-cloc/badges/16.0/pipeline.svg)](https://git.vauxoo.com/jarsa/odoo-cloc/pipelines)
[![Coverage Report](https://git.vauxoo.com/jarsa/odoo-cloc/badges/16.0/coverage.svg)](https://coverage.vauxoo.com/16-0-odoo-cloc)
...
3. Now we can use the t2d command. This will be accomplish by using the next
   command line:
    ...
    ```bash
    $ t2d git@git.vauxoo.com:jarsa/odoo-cloc.git 16.0
    ```
    ...
    ```bash
    ['${HOME}/t2d/script/git_git.vauxoo.com_jarsa_odoo-cloc.git/16.0/3.10/env_1_job_1',
     '${HOME}/t2d/script/git_git.vauxoo.com_jarsa_odoo-cloc.git/16.0/3.10/env_2_job_1']
    ```
...
4. The next step is to run the last mentioned scripts, we highly recommend to
   add the next extra arguments.
    ...
    ```bash
    ${HOME}/t2d/script/git_git.vauxoo.com_jarsa_odoo-cloc.git/16.0/3.10/env_2_job_1/10-build.sh --no-cache
    ${HOME}/t2d/script/git_git.vauxoo.com_jarsa_odoo-cloc.git/16.0/3.10/env_2_job_1/20-run.sh --entrypoint=bash
    ```
...
7. After this script is finish you can run the odoo server again on demmand
   using the next command:
    ...
    ```bash
    $HOME/odoo-16.0/odoo-bin -d openerp_test --db-filter=openerp_test
    ```
